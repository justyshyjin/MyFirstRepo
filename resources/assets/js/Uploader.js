var uploadHandler = function(){
  ...
  this.initate = function(options) {
      ...
      */ 
      if(this.isBrowserSupportedHtmlFive()){
         ...
         * @todo replace jquery extend with native Js
         */
  ...
  */    
  this.getXMLHttpRequest = function() {
    ...
    xhr.open("POST", this.file.dataset.action);
    xhr.setRequestHeader('X-CSRF-TOKEN' , this.csrfToken);
    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
    xhr.setRequestHeader('X-MARA-WEB-SERVICE', true);
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.setRequestHeader('X-MARA-PUBLIC-ACCESS-TOKEN', this.publicAccessToken);
    xhr.setRequestHeader('X-MARA-USER-ID', this.userId);
    xhr.setRequestHeader('X-MARA-ACCESS-TOKEN', this.accessToken);
  ...
  */  
  this.validateOptions = function() {
   /**
     ...
     * else throw the exception
     */
    if(!this.file.dataset.hasOwnProperty('action')){
      throw "Request URL should set in data attribute of the file";
    }
  ...
  */      
  this.processFile = function(fileList) {
    ...
    if(this.options.beforeUpload && typeof this.options.beforeUpload == 'function'){
       ...
       * @rule fileLimit 
       * @todo Display the Messgae to image in better way
       */ 
  ...
  */      
  this.validateImageFile = function(file) {
    ...
    /**
     * Valid image matched minimum expected resoultion
     * validImage minimumResolution rule is ignored since image loading is invalid
     * @rule minimumResolution 
     */ 
    /*if(this.options.validImage.minimumResolution && this.options.validImage.minimumResolution.indexOf('x') != -1){
      var resoultion = this.options.validImage.minimumResolution.split('x');

      if(resoultion.length == 2){
        var width  = parseInt(resoultion[0]);
        var height = parseInt(resoultion[1]);

        if(typeof width == 'number' && typeof height == 'number' && (width > image.width || height > image.width)){
          validateErrors.push('minimumResolution');
        }        
      }
    } */              
  ...
  * @param object file
  * @return boolean
  * @todo bring in actual condition for checking file type
  */  
  ...
  */     
  this.getErrorMessageFromResponse = function(response) {
    ...
    if(typeof message == 'object'){
      if(message.hasOwnProperty(this.requestFileName) && message[this.requestFileName].length > 0){
