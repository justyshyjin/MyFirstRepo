...
class CustomerAuthController extends BaseController
{
     ...
     * @return void
     */
    public function __construct(CustomerRepository $authRepository, Request $request, SubscriptionRepository $subscription)
    {
        $this->_customerRepository = $authRepository;
        $this->request = $request;
        $this->subscription = $subscription;
        $this->loginPath = 'auth' . DIRECTORY_SEPARATOR . 'login';
    ...
    public function registerOrLoginUser($socialUser, $type, $registeredMedium)
    {
        ...
        $user = $this->_customerRepository->registerSocialUser($userDetails, $type);
        if (is_object($user) && !empty ($user->id) && ($user->is_active)) {
            $this->request->session()->put('access_token', $user->access_token);
            $this->_customerRepository->checkAndAddExams($user);
    ...
    {
        $checkRandom = Customer::where('forgot_password', $random)->first();
    ...
    {
        $this->_customerRepository->savenewPassword($random);
    ...
    {
        $this->redirectTo = '/';
        $this->validate($request, [$this->username() => 'required|email', 'password' => 'required']);
        ...
        if (Auth::attempt($credentials, $request->has('remember'))) {
            ...
            if ($this->handleActiveUser()) {
                $authHandle = redirect($this->loginPath)->withInput($request->only($this->username(), 'remember'))->withErrors([$this->username() => trans("user::adminuser.login_inactive")]);
            } else {
                $authHandle = $this->handleUserWasAuthenticated($request, $throttles);
            }
            $user = Auth::user();
            $request->session()->regenerate();
            $user->access_token = $this->_customerRepository->randomCharGen(30);
        ...
        return redirect('/#/login')->withInput($request->only($this->username(), 'remember'))->withErrors([$this->username() => $this->getFailedLoginMessage()]);
    }
