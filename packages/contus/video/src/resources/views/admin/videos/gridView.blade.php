        ...
        <div class="tab-pane active" id="latest_video">
            <div class="tab_search clearfix">
                <div class="search_upload_btn pull-right">
                    <a data-ng-hide="tabSelected =='live_videos'" title="{{__('video::videos.upload_video')}}" href="javascript:void(0)" class="btn btn-primary upload_video pull-right" data-ng-click="vgridCtrl.showUploadOption()">
                        <i class="fa fa-cloud-upload" aria-hidden="true"></i>
                        {{__('video::videos.upload_video')}}
                    </a>
                    <a data-ng-show="tabSelected =='live_videos'" href="{{URL::to('/')}}/admin/videos/add" id="add_playlist" data-boot-tooltip="true" data-toggle="tooltip" class="btn btn-primary  pull-right mr10" >Add Stream</a>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="center">{{__('base::general.s_no')}}</th>
                    ...
                    <tbody>
                        <tr class="search_text">
                            ...
                            <td class="search_product">
                                <input type="text" class="form-control" data-ng-model="searchRecords.title" data-boot-tooltip="true" title="{{__('video::videos.enter_video_title')}}">
                            </td>
                            <td data-ng-hide="tabSelected =='live_videos'"></td>
                            <td data-ng-hide="tabSelected =='live_videos'"></td>
                            <td>
                                <select class="form-control mb15" data-ng-change="search()" data-ng-model="searchRecords.is_active" data-boot-tooltip="true" title="{{__('base::general.select_status')}}">
                            ...
                            <td data-ng-show="tabSelected =='live_videos'">
                                <select class="form-control mb15" data-ng-change="search()" data-ng-model="searchRecords.type" data-boot-tooltip="true" title="{{__('base::general.type')}}">
                                    <option value="all" selected>{{__('base::general.all')}}</option>
                                    <option value='wowza'>{{__('base::general.wowza')}}</option>
                                    <option value='private'>{{__('base::general.youtube_private')}}</option>
                                    <option value='public'>{{__('base::general.youtube_public')}}</option>
                                    <option value='unlisted'>{{__('base::general.youtube_unlisted')}}</option>
                                </select>
                            </td>
                            <td></td>
                            <td data-ng-hide="tabSelected !='live_videos'"></td>
                            <td></td>
                            <td>
                                <button type="button" class="btn search" data-ng-click="search()" data-boot-tooltip="true" title="{{__('base::general.search_filter')}}">
                                    <i class="fa fa-search"></i>
                                </button>
                                <button type="button" class="btn search" data-ng-click="gridReset()" data-boot-tooltip="true" title="{{__('base::general.reset')}}">
                                    <i class="fa fa-refresh"></i>
                                </button>
                            </td>
                        </tr>
                        <tr>
                            <td data-ng-if="noRecords" colspan="@{{heading.length + 2}}" class="no-data">{{__('base::general.not_found')}}</td>
                        </tr>
                        <tr data-ng-if="showRecords" data-ng-repeat="record in records track by $index" data-ng-show="showRecords" class="list-repeat" data-intialize-sidebar="">
                            ...
                            <td>
                                <span class="label label-success" ng-if="record.is_active == 1" style="cursor: pointer;" data-ng-click="vgridCtrl.updateStatus(record)" title="{{__('video::videos.deactivate_video')}}" data-boot-tooltip="true">{{__('video::videos.message.active')}}</span>
                                <span class="label label-danger" ng-if="record.is_active != 1" style="cursor: pointer;" data-ng-click="vgridCtrl.updateStatus(record)" title="{{__('video::videos.activate_video')}}" data-boot-tooltip="true">{{__('video::videos.message.inactive')}}</span>
                            </td>
                            <td data-ng-if="tabSelected =='live_videos'">
                            ...
                            <td data-ng-if="tabSelected =='live_videos'">
                                ...
                                <span class="label label-info" ng-if="record.liveStatus == 'complete'">{{__('video::videos.recorded')}}</span>
                                <span class="label label-success" ng-if="record.liveStatus == 'live'">Live</span>
                            </td>
                            <td data-ng-if="tabSelected !='live_videos'">
                                <span class="label label-primary" ng-if="record.job_status == 'Video Uploaded' || record.job_status == 'Submitted'">{{__('video::videos.uploaded_status')}}</span>
                                ...
                                <span class="label label-danger" ng-if="record.job_status == 'Error' || record.job_status == 'Canceled'">{{__('video::videos.error_status')}}</span>
...
<button ng-if="record.username !== ''" ng-show="record.liveStatus==='ready'||record.liveStatus==='stopped'" ng-click="startlivestream(record)" class="label label-success">Start</button>
                            ...
                            <td class="action">
                                <div class="column edit_table_icon">
                                    <a data-boot-tooltip="true" title="{{__('video::videos.edit_video')}}" class="table_action" href="{{url('admin/videos/details-video-edit')}}/@{{record.id}}">
                                        <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                    </a>
                                </div>
                                <span class="delete_table_icon" title="{{__('base::general.delete')}}" data-toggle="modal" data-target="#videoDeleteModal" data-ng-click="vgridCtrl.deleteSingleRecordVideos(record.id)" data-boot-tooltip="true">
...
<div id="myModalplaylist" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h5 class="modal-title">{{__('video::playlist.add_to_playlist')}}</h5>
                <p>Organize and display your subscription videos on the web and in your apps.</p>
            </div>
            <div class="modal-body">
