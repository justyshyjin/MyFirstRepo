...
<div class="product order_list" data-ng-controller="ViewVideoplaylistsController as vvideoplaylistsCtrl" data-ng-init=vvideoplaylistsCtrl.fetchData('{{$id}}')>
    ...
    <div class="contentpanel clearfix video-conatiner" data-ng-if="!vvideoplaylistsCtrl.notFoundFlag">
        <div class="video-list-grid">
            <div class="pagination_bredrumbs clearfix">
                <h4 class="pull-left">
                    {{trans('video::playlist.playlist')}} :
                    <span>
                        <i>@{{vvideoplaylistsCtrl.videoplaylists.name}}</i>
                    </span>
                </h4>
                <ul data-ng-hide="true" data-ng-if="vvideoplaylistsCtrl.videoplaylists.videos != ''" class="pull-right">
                    <li>
        ...
        <div data-ng-if="vvideoplaylistsCtrl.videoplaylists.videos == ''" style="text-align: center; width: 100%; margin-top: 15px;" colspan="@{{heading.length + 2}}" class="no-data">{{trans('base::general.not_found')}}</div>
        <ul class="video-conatiner-list" data-ng-if="vvideoplaylistsCtrl.videoplaylists.videos != ''" data-ng-show="vvideoplaylistsCtrl.videoGridView">
            <li data-ng-repeat="record in vvideoplaylistsCtrl.videoplaylists.videos track by $index">
...
<script src="{{$getVideoAssetsUrl('js/playlists/videos.js')}}"></script>
@endsection
