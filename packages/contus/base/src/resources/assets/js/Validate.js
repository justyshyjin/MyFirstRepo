var baseValidator = {
    ...
    validateAngularForm : function ( form, scope, module ) {
        ...
        angular.element( form ).find( 'textarea,select,input:not(input[type="submit"],input[type="file"],input[data-ignore-validation],input[data-unique])' ).each( function () {
            ...
            if ( module ) {
                angular.extend( scope.errors [module], baseValidator.getFormattedErrorMessage( validationErrors, scope.errors [module], this ) );
            } else {
    ...
    getFormattedErrorMessage : function ( validationErrors, errors, element ) {
        ...
        if ( angular.equals( {}, validationErrors ) ) {
            if ( index && parent && errors.hasOwnProperty( parent ) && errors [parent].hasOwnProperty( index ) && errors [parent] [index].hasOwnProperty( element.getAttribute( 'name' ) ) ) {
    ...
    /**
     * get dom element object by name if index is set get the element by id
     * 
     * @param string name
     * @param string index
     * @return object
    ...
    setErrorMessageForElement : function ( rule, element, params ) {
        var eleName = element.getAttribute( 'name' );
        var displayName = element.getAttribute( 'data-validation-name' ) ? element.getAttribute( 'data-validation-name' ) : this.formatName( element.getAttribute( 'name' ).replace( /[0-9]/g, '' ) );
            ...
            switch ( rule ) {
                case 'max':
                    if ( typeof message == 'object' && element.getAttribute( 'type' ) == "number" && message.hasOwnProperty( 'numeric' ) ) {
                        message = message.numeric.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /:max/g, this.params.hasOwnProperty( 'max' ) ? this.params.max : '' );
                    } else if ( typeof message == 'object' && message.hasOwnProperty( 'string' ) ) {
                        message = message.string.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /:max/g, this.params.hasOwnProperty( 'max' ) ? this.params.max : '' );
                    }
                    break;
                case 'min':
                    if ( typeof message == 'object' && message.hasOwnProperty( 'string' ) ) {
                        message = message.string.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /:min/g, this.params.hasOwnProperty( 'min' ) ? this.params.min : '' );
                    }
                    break;
                case 'required_unless':
                    message = message.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /:other/g, this.ucfirst( this.formatName( this.params.required_unless.inputName ) ) ).replace( /:value/g,
                            this.params.required_unless.hasOwnProperty( 'label' ) ? this.params.required_unless.label : '' );
                    break;
                case 'required_if':
                    message = message.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /:other/g, this.ucfirst( this.formatName( this.params.required_if.inputName ) ) ).replace( /:value/g, this.params.required_if.hasOwnProperty( 'label' ) ? this.params.required_if.label : '' );
                    break;
                case 'required_with':
                    message = message.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /:values/g, this.ucfirst( this.formatName( this.params.required_with.inputName ) ) ).replace( /:value/g, this.params.required_with.hasOwnProperty( 'label' ) );
                    break;
                case 'same':
                    if ( this.params.same.inputName == 'userpassword' ) {
                        message = message.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /:other/g, this.ucfirst( 'password' ) ).replace( /:value/g, this.params.same.hasOwnProperty( 'label' ) );
                    } else {
                        var value = angular.element( 'input[name="' + this.params.same.inputName + '"]' ).attr( 'data-validation-name' );
                        value = ( value ) ? value : this.formatName( this.params.same.inputName );
                        message = message.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /:other/g, this.ucfirst( value ) ).replace( /:value/g, this.params.same.hasOwnProperty( 'label' ) );
                    }
                    break;
                case 'different':
                    message = message.replace( /:attribute/g, this.ucfirst( displayName ) ).replace( /and :other/g, '' ).replace( /:value/g, this.params.different.hasOwnProperty( 'label' ) );
                    break;
                case 'url':
        ...
        if ( this.params.hasOwnProperty( 'same' ) && typeof this.params.same == 'object' && this.params.same.hasOwnProperty( 'inputName' ) ) {
        ...
        if ( this.params.hasOwnProperty( 'required_if' ) && typeof this.params.required_if == 'object' && this.params.required_if.hasOwnProperty( 'inputName' ) && this.params.required_if.hasOwnProperty( 'inputValue' ) ) {
            ...
            if ( dependendElement && dependendElement.value == this.params.required_if.inputValue && !this.validateRequired( element ) ) {
        ...
        if ( this.params.hasOwnProperty( 'required_unless' ) && typeof this.params.required_unless == 'object' && this.params.required_unless.hasOwnProperty( 'inputName' ) && this.params.required_unless.hasOwnProperty( 'inputValue' ) ) {
            ...
            if ( typeof dependendValue == 'number' && parseInt( dependendElement.value ) != parseInt( this.params.required_unless.inputValue ) && !this.validateRequired( element ) ) {
                /**
                 * set the selected value label so we can use it for error message
                 */
        ...
        if ( this.params.hasOwnProperty( 'required_with' ) && typeof this.params.required_with == 'object' ) {
            ...
            if ( dependendElement && typeof dependendElement == 'object' && dependendElement.value.trim().length > 0 && !this.validateRequired( element ) ) {
                return false;
            }
        }
    ...
    validateDate : function ( element ) {
        ...
        if ( element.hasAttribute( 'data-unique' ) ) {
            ...
            xhr.addEventListener( "load", function () {
                if ( this.status != 200 && baseValidator.scope && baseValidator.scope.hasOwnProperty( 'errors' ) ) {
                    baseValidator.scope.errors [eleName] = {has : true,message : baseValidator.getErrorMessageByRule( 'unique' ).replace( /:attribute/g, baseValidator.ucfirst( eleName ) )};
        ...
        if ( dependendParams ) {
            ...
            if ( typeof dependendElement == 'object' ) {
                ...
                if ( typeof dependendValue == 'number' && parseInt( dependendElement.value ) != parseInt( inputValue ) ) {
                    return true;
                }
            }
        }
