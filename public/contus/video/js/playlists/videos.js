...
var videoplaylists = angular.module('videoplaylists',[]);

videoplaylists.directive('baseValidator',validatorDirective);

videoplaylists.factory('requestFactory',requestFactory);

videoplaylists.directive('bootTooltip',function(){
	return {
		restrict: 'A',
...
videoplaylists.controller('ViewVideoplaylistsController',['$window','$scope','$rootScope','requestFactory','$timeout',function(win,scope,$rootScope,requestFactory,$timeout){
  var self = this;
  scope.errors = {};
  scope.currentPage = 1;
  this.videoplaylists = {};
  ...
  this.routeURL='playlists';

  this.fetchData = function(id) {
    requestFactory.get(requestFactory.getUrl('playlists/'+id,{page : scope.currentPage}),function(response){
      this.videoplaylists = response.message.playlist;
      this.videoplaylists.videos = response.message.videos.data;
      requestFactory.toggleLoader();
      scope.totalRecords = parseInt(response.message.videos.total);
      scope.rowsPerPage  = parseInt(response.message.videos.per_page);
      scope.currentPage  = parseInt(response.message.videos.current_page);
  ...
  scope.loadRecords = function(pageNumber,orderStatus) {
    ...
    self.fetchData(self.videoplaylists.id);
    requestFactory.toggleLoader();
  }
  ...
  this.paginate = function(totalLinks) {
    ...
    this.removeVideosFromCollection = function(id) {
        this.videoId = [id];
        this.collectionId = self.videoplaylists.id;
    };
    ...
    this.deleteRecordsVideosFromCollection = function(videoId, playlistId) {
        ...
        scope.deleteRequest = requestFactory.post(requestFactory.getUrl('playlist/delete'),angular.extend({},{selectedVideos:videoId.join(','),id:playlistId},scope.requestParams),function(data){
        	if(self.videoplaylists.videos.length - deleteIdLength == 0 ) {
	            scope.currentPage = 1;
	        }
	        self.fetchData(self.videoplaylists.id);
        });
...
angular.element(document).ready(function() {
  angular.bootstrap(document, ['videoplaylists']);
});
